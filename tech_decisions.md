...
- **Plan checklist hygiene (required):** if implementation starts from a `plans/0N_*.md` doc, keep that plan's own checkboxes current as work lands (work breakdown, validation, and docs-on-landing). Do this in the same change set as the feature so roadmap status and shipped behavior cannot drift.
- **No git-command workflow:** for this project workflow, do not use git commands as part of feature implementation or plan maintenance tasks. Treat repository state as managed outside the implementation steps.
...
## Child tasks and rollups (plan 07 — landed)
- **Hierarchy depth is intentionally capped at 2.** A task may have a single optional `parent_task_id`, but the service layer refuses any selection that would create a cycle, make a child into a parent, or make a parent into a child. In practice that means "root task plus direct children" only: no grandchildren, no trees. This keeps list rendering, reports, and mental models simple while still solving the main decomposition use case ("one umbrella task with several independently-trackable child tasks").
- **Picker and warning behavior follow existing UX patterns.** Parent assignment uses a dedicated searchable picker dialog, matching the dependency picker interaction instead of introducing a second selection style. Candidates are pre-pruned before display and the picker respects the Tasks-tab closed-task visibility toggle so closed tasks appear only when the user has opted in. Closing a parent never cascades to children; instead the UI shows a gentle confirmation when open children remain, preserving explicit user control over child lifecycles.
